...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Pacman and the two ghosts can be anywhere in the grid
    possibleLocations = list()
    for x in range(0, X_RANGE):
        for y in range(0, Y_RANGE):
            possibleLocations = possibleLocations + [ (x,y) ]

    variableDomainsDict[GHOST0] = variableDomainsDict[PAC] = variableDomainsDict[GHOST1] = possibleLocations

    # Calculating max distance
    maxDistance = -1
    for pacman in possibleLocations:
        for ghost in possibleLocations:
            maxDistance = max( maxDistance, manhattanDistance(pacman, ghost) )

    # Observations equal to Manhattan distances of Pacman to ghosts +- noise
    variableDomainsDict[OBS0] = variableDomainsDict[OBS1] = range(0, maxDistance + MAX_NOISE + 1)
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        currentFactorsNotToJoin = bayesNet.getAllCPTsWithEvidence( evidenceDict )

        for eliminationVariable in eliminationOrder:
            currentFactorsNotToJoin, joinedFactor = joinFactorsByVariable( currentFactorsNotToJoin, eliminationVariable )

            if len( joinedFactor.unconditionedVariables() ) >= 2:
                currentFactorsNotToJoin.append( eliminate(joinedFactor, eliminationVariable) )

        # Normalizing
        normalizedResult = normalize(joinFactors(currentFactorsNotToJoin))
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        if totalSum is None or totalSum == 0:
            return

        # Normalizing according to the normal sum
        for i in self.keys():
            self[i] = self[i] / totalSum
        "*** END YOUR CODE HERE ***"
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Setting random value between [0, totalSum]
        randomVal = random.random() * self.total()

        # Checking which sample it is
        totalSum = 0
        for i in self.keys():
            if randomVal < totalSum + self[i]:
                return i
            totalSum = totalSum + self[i]
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Corner case ---> Jail Situation
        if ghostPosition == jailPosition:
            return noisyDistance == None

        # if noisy distance is none return 0 otherwise return getObservationProbability()
        return 0 if noisyDistance is None else busters.getObservationProbability( noisyDistance, manhattanDistance(pacmanPosition, ghostPosition) )
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Getting position of Pac-man
        pac = gameState.getPacmanPosition()

        # Getting position of jail
        jail = self.getJailPosition()

        # Update the belief at every position
        for pos in self.allPositions:
            self.beliefs[pos] = self.beliefs[pos] * self.getObservationProb( observation, pac, pos, jail )
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Creating a result distribution
        result = DiscreteDistribution()

        # update the belief at every position after one time step
        for oldPos in self.allPositions:
            newPos = self.getPositionDistribution(gameState, oldPos)
            for i in newPos:
                result[i] = result[i] + self.beliefs[oldPos] * newPos[i]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        # Calculate length of the legal positions
        length = len( self.legalPositions )
        for particle in range(0, self.numParticles):
            index = particle % length
            # Distribute evenly
            self.particles = self.particles + [ self.legalPositions[index] ]
        "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        # Convert particles into distribution
        for particle in self.particles:
            # Calculates particle count
            count = 0
            for particle2 in self.particles:
                if particle2 is particle:
                    count = count + 1
            # Add its count
            beliefDistribution[particle] = count
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # pac and jail are the same as question 6
        # Getting position of Pac-man
        pac = gameState.getPacmanPosition()
        ...
        # Calculates weight of each particle
        weightDistribution = DiscreteDistribution()
        for particle in self.particles:
            weightDistribution[particle] = weightDistribution[particle] + self.getObservationProb(observation, pac, particle, jail)

        # Calculates total sum
        totalSum = weightDistribution.total()

        # All particles not have 0 weight
        if totalSum != 0:
            # Normalizing
            ...
            #Resampling
            samples = list()
            for n in range(0, self.numParticles):
                samples.append( weightDistribution.sample() )

            # New list of particles
            self.particles = samples

        # All particles have 0 weight --> Corner case
        else:
            self.initializeUniformly(gameState)
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Below code also works, but you want us to use newPosDist = self.getPositionDistribution(gameState, oldPos)
